...
| 命令 | 别名 | 含义 |
|---|---|---|
| `flc devices list` | `-l` | 列出已连接的 Apple 设备（名称、型号、iOS、UDID、USB/网络）。 |
...
| `flc ddi install` | `-i` | 对已连接 iPhone 个性化并安装 DDI（通常首次 `set` 时自动完成）。 |
...
- 它在同一连接上**每 15 秒**重新下发一次位置，让手机持续报告虚拟位置，而不是漂回
  真实 GPS。
...
- **更新的 iOS 提示 DDI build 不匹配** —— 每个 iOS beta/正式版可能需要对应 DDI。
  ...
  `flc ddi sync`。
- **位置不对 / 不动 / 跳回真实 GPS** —— 保持窗口必须一直开着；它每 15 秒重新下发，
  断线会重建隧道。若仍跳回，先 `flc set own` 再重新 `set`。部分 App 会缓存位置，
  请重新打开它们。
